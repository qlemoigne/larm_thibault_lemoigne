...
class ObjectsDetector(Node):
    ...
    def __init__(self):
        ...
        self.color = 8 # Hue value for bright orange 
        
        # We can adjust saturation to include more images
        self.lo = np.array([self.color - 8, 240, 240]) # Slightly darker and dimmer orange
        self.hi = np.array([self.color + 8, 255, 255]) # Slighty lighter and brighter orange 
    ...
    def onImage(self, data: Image):
        ...
        try:
            ...
            image2 = cv2.erode(cv_image, self.kernel, iterations=1)

            mask = cv2.inRange(image2, self.lo, self.hi) # bit-mask selecting desired pixel
            ...
            if foundArea.isEmpty() == False:
                cv2.rectangle(image2, (foundArea.minx, foundArea.miny), (foundArea.maxx, foundArea.maxy), (0, 0, 255), 2)
